...
namespace A8
{
    public class Q2Airlines : Processor
    {
        ...
        public virtual long[] Solve(long flightCount, long crewCount, long[][] info)
        {
            ...
            int flight_count = (int)flightCount; // cities
            int crew_count = (int)crewCount;
            ...
            for (int i = 0; i < flight_count; ++i) // left to right edges
            {
                ...
                {
                    residualGraph[i + 1, flight_count + 1 + j] = (int)info[i][j];
                }
            }
            maxFlow(residualGraph, flight_count + crew_count + 2);
            long[] ans = new long[flight_count]; // matching
